 ...
 */
        ...
        // 모든 노드를 방문하면 종료되기 때무넹, 노드의 갯수 만큼만 반복한다,.
        for (int i = 0; i < nodes; i++) {
            // 3-1. 현재 알고있는 거리 비용 중 최단 비용 노드를 선택한다.
            int minDist = Integer.MAX_VALUE;
            int minDistNode = -1;
            for (int j = 0; j < nodes; j++) {
                // 3-1-1. 방문하지 않았고, 거리 비용이 최소인 지를 확인한다.
            ...
            for (int j = 0; j < nodes; j++) {
                ...
                //       vs 현재 노드까지의 거리 (dist[minDistNode] + 현재 노드~인접 노드까지의 거리 (cost)
                if (dist[j] > dist[minDistNode] + cost ) {
                    dist[j] = dist[minDistNode] + cost;
                }
            }
        }
        // 4. 최종적으로 각 정점에 도달하는데 걸리는 비용을 출력
